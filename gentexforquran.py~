...
targets=open('qus.tex','w')
transm=open('en.maududi.txt','r')
transp=open('en.pickthall.txt','r')
transs=open('en.sahih.txt','r')
...
for x in range(114):
    targets.write("\\chapter{%s}\n" % (suraname[x]) )
    targets.write("\\begin{Arabic}\n\\Huge{\\centerline{\\basmalah}}\\end{Arabic}\n")
    count=0
    while(count<surasize[x]):
        targets.write("\\flushright{\\begin{Arabic}\n")
        targets.write("\\quranayah[%d][%d]\n" %(x+1,count+1))
        targets.write("\\end{Arabic}}\n")
        count=count+1
        ab=transs.readline()
        targets.write("\\flushleft{\n")
        targets.write(ab)
        targets.write("}\n")
